...
import { UserRole, MaterialRequest, Notification, RequestStatus } from './types';
import { USER_ROLES, MOCK_REQUESTS } from './constants';
 ...
 * @param oldRequest The request state before the update.
 * @param updatedRequest The request state after the update.
 * @returns An array of notification message strings.
 */
const generateUpdateNotifications = (oldRequest: MaterialRequest, updatedRequest: MaterialRequest): string[] => {
    const messages: string[] = [];

    // 1. Check for overall status change
    ...
    // 3. Check for PR item status changes (more robustly)
